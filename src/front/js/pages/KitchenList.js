...
export const KitchenList = () => {
  ...
  const [elapsedTimes, setElapsedTimes] = useState(Date.now());
  ...
  useEffect(() => {
    if (store.orders) {
      ...
      store.orders.forEach((order) => {
        ...
        initialElapsedTimes[order.id] = initialElapsedTimes[order.id] || 0;
      });
  ...
  const toggleItemCompleted = (orderId, itemId) => {
    ...
    updatedCompletedItems[orderId][itemId] = !updatedCompletedItems[orderId][itemId];
